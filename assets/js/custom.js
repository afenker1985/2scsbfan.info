		...
		$.getJSON( "assets/db/albums.json", function( data ) {
			...
			$.each( data, function (key, val) {
				...
				if (Number.isInteger(parseInt(key)) && val["is_active"]==1) {
					...
					albums.push( "<span class='mu-filter-imglink' id='list-track-" + key + "'><i class='fa fa-question-circle'></i></span>" );
					albums.push( "</div>" );
					albums.push( "</div>" );
...
document.getElementById('[id*="list-track-"]').onclick = create_track_listing( this.id );
